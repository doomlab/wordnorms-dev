...
import db from "db"
import { BorderlineActions } from "../BorderlineActions"

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const paper = await db.paper.findUnique({ where: { id: Number(id) }, select: { title: true } })
  return { title: paper ? `${paper.title} – Borderline Review` : "Borderline Review – Admin" }
}

const cap = (s: string) => s.charAt(0).toUpperCase() + s.slice(1)

export default async function AdminExcludedDetailPage({
  params,
  searchParams,
}: {
  ...
  const nextHref = nextFirst
    ? `/admin/excluded/${nextFirst}${restNext.length ? `?next=${restNext.join(",")}` : ""}`
    : "/admin/excluded"
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        ...
        <a
          href={`/admin/papers/${paper.id}?from=/admin/excluded/${paper.id}`}
          className="btn btn-outline btn-sm"
        >
          Edit paper
        </a>
