...
import toggleDatasetFavorite from "../(dashboard)/mutations/toggleDatasetFavorite"

export function DatasetFavoriteButton({
  bibtex,
  initialFavorited,
  isLoggedIn = true,
}: {
  ...
  const [toggle] = useMutation(toggleDatasetFavorite)
  const router = useRouter()

  const handleClick = async (e: React.MouseEvent) => {
    e.preventDefault()
    try {
      const result = await toggle({ bibtex })
